...
class AgentCPPlugin(private val context: Context) : MethodCallHandler {
    ...
    companion object {
        ...
        const val CHANNEL_NAME = "com.example.evol/agentcp"
    }
            ...
            try {
                ...
                try {
                    ...
                    if (msg.contains("request already exists") || msg.contains("code=1012")) {
                        Log.w(TAG, "groupJoinByUrl: request already exists, clearing and retrying...")
                        if (groupId.isNotEmpty()) {
                            try {
                                agent.groupLeaveGroup(groupId)
                                Log.i(TAG, "groupJoinByUrl: cleared old membership for $groupId")
                            } catch (leaveErr: Exception) {
                                Log.w(TAG, "groupJoinByUrl: leave failed (ok): ${leaveErr.message}")
                            }
                        }
                        requestId = agent.groupJoinByUrl(groupUrl, inviteCode, message)
                        Log.i(TAG, "groupJoinByUrl: retry succeeded, requestId=$requestId")
                    } else {
                        throw e
                    }
                }

                val joinStatus = detectJoinStatus(agent, groupId, inviteCode, requestId)
